...
• In 2018, it was estimated that 46% of ambulance response to serious medical emergencies took longer than 10 minutes in NYC. <br>


**Inefficiences in NYC EMS System** <br>
• The Citizen’s Budget Commission described major inefficiencies in NYC’s EMS system in a recent report, Reviving EMS: Restructuring Emergency Medical Services in NYC. These inefficiencies included: <br>
...
We narrowed the data set for research purposes in the following ways: <br>
...
time_boro = ems_data_clean %>%
  ...
  plot_ly(x = ~neighbourhood, y = ~mean_time/60, color = ~arrival_outcome, group = ~arrival_outcome, type = "bar", colors = "Paired") %>% 
...
pie_chart_all = ems_data_clean %>% 
  ...
  count(initial_call_type, sort = "TRUE", name="n") %>%
   plot_ly(labels = ~initial_call_type, values = ~n, type = "pie",
          ...
          insidetextfont = list(color ="YlGnBu")) %>% 
...
**The highest percentage of initial call types are those labeled as sick, injured,  psychiatric-related, drug/ alcohol abuse, and unconscious.** Many of these are non-threatening and may not be deemed “medical emergencies”. **Meanwhile, nearly 75% of cardiac arrest calls will be dead upon arrival.**

Reponses times vary by call type and times also vary by neighborhood within call type (e.g., Central Harlem has the slowest response times for both sick and drug/alcohol-related calls). More resources should be allocated to threatening medical emergencies. 

<br> 

## Strengths and Limiitations 
• Only analyzed response times in Manhattan (2018). <br>
• Data set did not include variables for traffic, location of nearest fire station/EMS station, or demographic data (age, race, income).<br>
• Our data is collected only after the first 911 call taker has passed the call along to the FDNY EMS (Incident creation). Thus, it does not include the amount of time the caller spent explaining their emergency to the dispatcher before being transferred to the FDNY. <br>

<br> 

## Conclusion
• We agree with the Citizen’s Budget Commission’s report that there are inefficiencies in NYC’s EMS system. <br>

• Based on our results, we believe that while improving overall EMS time is ideal, attention should be focused on improving times in certain neighborhoods and ensuring more resources are allocated to these neighborhoods (Central Harlem, East Harlem, Inwood/Washington Heights, and Upper West Side). Further efforts should be made to decrease response times in life-threatening medical emergencies and during certain time frames (winter and summer months). <br>
